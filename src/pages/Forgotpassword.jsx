...
/**
 * OPD Decision Support System — Forgot Password
 *
 * Three-step flow (all on one page, no full navigations):
 *   Step 1 → User enters their registered email.
 *            POST /auth/forgot-password  { email }
 *            Backend generates a 6-digit OTP, saves it (hashed) + expiry to DB,
 *            and sends it via email/SMS.
 *
 *   Step 2 → User enters the 6-digit OTP.
 *            POST /auth/verify-otp  { email, otp }
 *            Backend verifies hash + expiry, returns a short-lived resetToken.
 *
 *   Step 3 → User enters + confirms a new password.
 *            POST /auth/reset-password  { resetToken, newPassword }
 *            Backend verifies resetToken, hashes newPassword, saves to DB,
 *            invalidates the token, redirects user to login.
 *
 * Visual identity matches Login.jsx (Space Grotesk / Inter / JetBrains Mono,
 * teal #1C6E74, dark navy #0E2A38, warm off-white background).
 */
...
export default function ForgotPassword() {
    ...
    const strengthColor = ["", "#D64C3A", "#D69A3A", "#2D8C5E", "#1C6E74"][strength ?? 0];
    ...
    const handleSendOtp = async () => {
        ...
        try {
            ...
            setError("No account found for that email. Try again.");
    ...
    const handleResetPassword = async () => {
        ...
        if ((strength ?? 0) < 2) { setError("Choose a stronger password."); return; }
        setLoading(true);
        try {
    ...
    return (
        <div className="min-h-screen w-full flex bg-[#FBF8F3] font-[Inter]">
            <style>{`
                ...
                .opd-ecg-track  { animation: ecg-scroll 4.5s linear infinite; }
                .opd-pulse-dot  { animation: pulse-dot 1.8s ease-in-out infinite; }
                .opd-fade-in    { animation: fade-in-up .35s ease-out; }
                .opd-grid-bg {
                    background-image:
            ...
            <div className="hidden lg:flex lg:w-[44%] relative flex-col justify-between bg-[#0E2A38] opd-grid-bg overflow-hidden px-12 py-12">
                <div>
                    ...
                    <p className="mt-4 text-[#C9D9DA] text-sm leading-relaxed max-w-xs">
                        ...
                        Use it to set a new password — the code expires in 10 minutes.
                    </p>
                        ...
                        {stepMeta.map(({ n, label }) => {
                            ...
                            return (
                                <div key={n} className="flex items-center gap-3">
                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-[JetBrains_Mono] shrink-0 transition-colors ${
                                        done   ? "bg-[#1C6E74] text-white" :
                                        active ? "bg-white text-[#0E2A38]" :
                                                 "bg-white/10 text-white/40"
                                    }`}>
                                        {done ? "✓" : n}
                                    </span>
                                    <span className={`text-sm ${active ? "text-white font-medium" : done ? "text-[#9FD8D6]" : "text-white/30"}`}>
                                        {label}
                                    </span>
                                </div>
                            );
                ...
                <div className="relative w-full h-24 overflow-hidden">
                    <div className="absolute top-1/2 -translate-y-1/2 flex opd-ecg-track" style={{ width:"200%" }}>
                        {[0,1].map((k) => (
                            <svg key={k} viewBox="0 0 600 100" className="w-1/2 h-24" preserveAspectRatio="none">
                                <path d="M0,55 L120,55 L140,20 L160,90 L180,10 L200,55 L600,55"
                                    fill="none" stroke="#FF6F61" strokeWidth="2.5"
                ...
                <div className="w-full max-w-sm opd-fade-in">
                    ...
                    {step === 2 && (
                        <>
                            ...
                            <p className="mt-1.5 text-sm text-[#6B7A7E] mb-7">
                                Sent to <span className="font-medium text-[#0E2A38]">{email}</span>
                            </p>
                            ...
                            {/* OTP boxes */}
                            <div className="flex gap-2.5 justify-between mb-6" onPaste={handleOtpPaste}>
                                {otp.map((digit, idx) => (
                                    <input
                                        key={idx}
                            ...
                            <p className="mt-2 text-center text-xs text-[#9AA6AA]">
                                Wrong email?{" "}
                                <button onClick={() => { setStep(1); setError(""); setInfo(""); }}
                        ...
                        <>
                                    ...
                                    <div className="flex gap-1 mb-1">
                                        {[1,2,3,4].map((n) => (
                                            <div key={n} className="flex-1 h-1 rounded-full transition-colors"
                                                style={{ backgroundColor: (strength ?? 0) >= n ? strengthColor : "#E1E6E5" }} />
                                        ))}
                                    </div>
                                    <p className="text-[11px]" style={{ color: strengthColor }}>
                                        {strengthLabel} — use uppercase, numbers & symbols for a stronger password.
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="relative mb-6">
                                <input
                                    ...
                                    className={`w-full border focus:ring-1 outline-none p-3 pr-11 rounded-lg text-sm bg-white ${
                            ...
                            {confirmPassword && confirmPassword !== newPassword && (
                                <p className="text-xs text-[#D64C3A] -mt-4 mb-4">Passwords don't match.</p>
                            )}
...
function Banner({ type = "error", children }) {
    ...
    return (
        <div className={`mb-5 px-4 py-3 rounded-lg border text-sm opd-fade-in ${styles[type]}`}>
            {children}
        </div>
    );
}
